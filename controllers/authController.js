...
exports.verifyVerificationCode = async (req,res) => {
    ...
    try{
        const { error, value } = verifyVerificationCode.validate({ email, providedCode });
        if (error) {
            return res
                .status(401)
                .json({ success: false, message: error.details[0].message })
        ...
        const existingUser = await User.findOne({email}).select('+verificationCode +verificationCodeValidation'); //find the issue 
